...
namespace Core {
	...
	RenderingFrame* RenderingFrame::InstanseDiscription(glm::ivec2 size) {
		discription_frame = new RenderingFrame(size);

		return discription_frame;
	}

	void RenderingFrame::DeleteDiscription() {
		if (discription_frame)
			delete discription_frame;
	}

	RenderingFrame::RenderingFrame(glm::ivec2 win_size) : source_video_stream{} {
		...
		source_video_stream = new Camera(win_size);

		source_video_stream->ConnectMouseTraking();
	}
